...
public abstract class AbstractUserSessionService implements UserSessionService {
	...
	protected abstract PersistenceUserSession createPersistenceUserSession(User user, Set<String> additionalRoles, UserSessionType type,
			TimeSpan maxIdleTime, TimeSpan maxAge, Date fixedExpiryDate, String internetAddress, Map<String, String> properties,
	...
	@Required
	public void setSystemUserSession(UserSession systemUserSession) {
	...
	@Override
	public Maybe<UserSession> createUserSession(User user, Set<String> additionalRoles, UserSessionType type, TimeSpan maxIdleTime, TimeSpan maxAge,
		...
		PersistenceUserSession pUserSession = createPersistenceUserSession(user, additionalRoles, type, maxIdleTime, maxAge, fixedExpiryDate,
				internetAddress, properties, acquirationKey, blocksAuthenticationAfterLogout);
		return Maybe.complete(mapToUserSession(pUserSession));
	}

	@Override
	public Maybe<UserSession> findUserSession(String sessionId) {
		if (systemUserSession.getSessionId().equals(sessionId)) {
			log.trace(() -> "Fetching system session '" + sessionId + "'; Returning: " + systemUserSession);
			return Maybe.complete(systemUserSession);
		}
	...
	protected PersistenceUserSession initPersistenceUserSession(PersistenceUserSession pUserSession, User user, Set<String> additionalRoles,
			TimeSpan maxIdleTime, TimeSpan maxAge, Date fixedExpiryDate, String internetAddress, Map<String, String> properties,
			String acquirationKey, boolean blocksAuthenticationAfterLogout, UserSessionType userSessionType, Date now) {
