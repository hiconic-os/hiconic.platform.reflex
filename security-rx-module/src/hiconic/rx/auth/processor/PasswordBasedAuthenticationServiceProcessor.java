...
// ============================================================================
package hiconic.rx.auth.processor;
...
/**
 * <p>
 * The password comparison maybe influenced by cryptography related meta data (e.g.: {@link PropertyCrypting}) associated with the {@link User}'s
 * <b>password</b> property.
 * 
 *
 * @param <T>
...
public abstract class PasswordBasedAuthenticationServiceProcessor<T extends UserPasswordCredentials>
	...
	private CmdResolver userModelCmdResolver;

	@Required
	public void setUserModelCmdResolver(CmdResolver userModelCmdResolver) {
		this.userModelCmdResolver = userModelCmdResolver;
	}
	 ...
	 * 
	 * <p>
	 * The {@link Codec} configured to this property is used, when the {@link User} {@code password} property is not marked with a
	 * {@link PropertyCrypting} metadata, to encode the user provided password before forming the authentication query which includes password in its
	 * conditions.
	 * 
	 * <p>
	 * If the {@link User} {@code password} property is marked with a {@link PropertyCrypting} metadata, the {@link #passwordEncoder} property is not
	 * used.
	 * 
	 ...
	 * 
	 * <p>
	 * The {@link CryptorProvider} configured to this property is used, when the {@link User} {@code password} property is marked with a
	 * {@link PropertyCrypting} metadata, to provide a {@link Cryptor} capable of comparing the user provided password with the encrypted password
	 * from the {@link User} instance.
	 * 
	...
	protected Maybe<User> authenticate(UserIdentification userIdentification, String password) {
		...
		Cryptor userPasswordCryptor = getUserPasswordCryptor();

		// TODO: correlation UUID logging

		if (userPasswordCryptor == null) {
			Maybe<User> userMaybe = retrieveUser(userIdentification, encodeUserPassword(password));
	...
	private String encodeUserPassword(String password) {
		...
		try {
			// WTF is this? Codec implies it can be c
	...
	private Cryptor getUserPasswordCryptor() {
		PropertyCrypting propertyCrypting = userModelCmdResolver.getMetaData().entityClass(User.class).property(User.password)
				.meta(PropertyCrypting.T).exclusive();
