...
@Managed public class BrowserAcceptanceRxModuleSpace implements RxModuleContract {
	...
	@Override public void configureServiceDomains(ServiceDomainConfigurations configurations){
		...
		ServiceDomainConfiguration domain=configurations.byId(SecurityServiceDomain.security); domain.bindRequest(BrowserAcceptanceRequest.T,this::processor); domain.bindRequest(RequestBrowserAcceptance.T, this::requestProcessor);
		domain.configureModel(e->{map(e,ListBrowserAcceptances.T,"browser-acceptances",HttpRequestMethod.GET);map(e,RequestBrowserAcceptance.T,"browser-acceptances/request",HttpRequestMethod.POST);map(e,ApproveBrowserAcceptance.T,"browser-acceptances/approve",HttpRequestMethod.POST);map(e,RejectBrowserAcceptance.T,"browser-acceptances/reject",HttpRequestMethod.POST);map(e,RevokeBrowserAcceptance.T,"browser-acceptances/revoke",HttpRequestMethod.POST);map(e,ForgetBrowserAcceptance.T,"browser-acceptances/forget",HttpRequestMethod.POST);});
	}
	private void registerExtensions(){if(registered)return;registered=true;security.registerUserSessionOpeningVerificationExpert(verifier());security.registerUserSessionAccessVerificationExpert(verifier());webSecurity.registerRequestContextContributor(cookieContributor());webSecurity.registerLoginFlowExtension(loginFlowExtension());}
	...
	@Managed private BrowserAcceptanceProcessor processor(){return new BrowserAcceptanceProcessor(store(),config().getApproverRoles(),matcher(),platform.serviceProcessing().evaluator());}
	@Managed private ReasonedServiceProcessor<RequestBrowserAcceptance, BrowserAcceptance> requestProcessor(){return processor()::request;}
	@Managed private BrowserAcceptanceLoginFlowExtension loginFlowExtension(){return new BrowserAcceptanceLoginFlowExtension(platform.serviceProcessing().evaluator());}
}
