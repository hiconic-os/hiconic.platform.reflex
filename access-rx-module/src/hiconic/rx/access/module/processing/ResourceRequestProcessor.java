...
import hiconic.rx.access.api.AbstractDispatchingAccessRequestProcessor;
import hiconic.rx.access.api.AccessRequestContext;
import hiconic.rx.access.api.DispatchConfiguration;
...
public class ResourceRequestProcessor extends AbstractDispatchingAccessRequestProcessor<ResourceRequest, BinaryRetrievalResponse> {
	...
	private Maybe<GetBinaryResponse> get(AccessRequestContext<GetResource> context) {
		...
		getPayload.setDomainId(context.getDomainId());
		getPayload.setResourceSource(resourceMaybe.get().getResourceSource());
	...
	private Maybe<PipeResourcePayloadResponse> stream(AccessRequestContext<StreamResource> context) {
		...
		pipePayload.setDomainId(context.getDomainId());
		pipePayload.setResourceSource(resourceMaybe.get().getResourceSource());
		pipePayload.setCondition(request.getCondition());
		pipePayload.setCapture(request.getCapture());
		pipePayload.setRange(request.getRange());
